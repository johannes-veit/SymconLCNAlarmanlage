...
## Zusatztests Panik-Korrektur 0.1.3

1. Mit ausgewählter `Panik -> Status`-Integer-Rückmeldung darf keine Störung wegen fehlender Aktion erscheinen.
2. Alle ausgewählten LCN-Licht-Statusvariablen müssen `HasAction() = TRUE` liefern; andernfalls klare Konfigurationsstörung.
3. Alarmstart: nur aktuell AUS gemeldete Panik-Lichter werden EIN angefordert.
4. Bereits EIN gemeldete Panik-Lichter erhalten keinen Toggle-Befehl.
5. Quittierung während laufender EIN-Serie: verbleibende EIN-Aufträge werden verworfen; AUS-Serie übernimmt.
6. Automatisches Alarmende: alle aktuell EIN gemeldeten Panik-Lichter werden AUS angefordert.
7. Ein physischer Lichttaster EIN->AUS quittiert den Alarm; Hauptschalter der Alarmanlage bleibt EIN.
8. PanikQueue-Timer ist im Ruhezustand 0 ms.
